...
@labscript_device
class PulseBlaster_No_DDS_Old(PulseBlaster):

    description = 'generic DO only Pulseblaster OLD SCHOOL, cannot ask if done'
...
@BLACS_tab
class Pulseblaster_No_DDS_Old_Tab(DeviceTab):
    # Capabilities
    num_DO = 24
    def __init__(self,*args,**kwargs):
        if not hasattr(self,'device_worker_class'):
            self.device_worker_class = PulseblasterNoDDSOldWorker
...
@BLACS_worker
class PulseblasterNoDDSOldWorker(Worker):
    core_clock_freq = 100
    def init(self):
        ...
        self.pb_read_status = self.z_pb_read_status
    ...
    def transition_to_buffered(self,device_name,h5file,initial_values,fresh):
        ...
        with h5py.File(h5file,'r') as hdf5_file:
            ...
            #add an extra second to total time for safety
            self._total_time = sum([i[3] for i in pulse_program]) * 1e-9 + 1
    ...
    def z_pb_read_status(self):
        """function to get status of board which cannot be queried
        requres the worker to be started and length of method to be known
        will look at the total length of time"""
        if time.time() - self._start_time < self._total_time:
            _run = True
            _wait = False
        else:
            _wait = True
            _run = False
        return {"stopped":False,"reset":True,"running":_run, "waiting":_wait}
    ...
    def check_status(self):
        if self.waits_pending:
            try:
                ...
                pass
        return self.z_pb_read_status(), self.waits_pending
...
@runviewer_parser
class PulseBlaster_No_DDS_OldParser(PulseBlasterParser):
    num_dds = 0
    num_flags = 24
